/* ---------- 1. データ ---------- */
// Mode3 では不要な Mode1 カリキュラム要素を削除しました。
// （lines, window.lines, kana2roma など）
...
function advanceToNextChar() {
    ...
    // Mode1: 同じ文字を5回打ち終わるまで charIdx を進めない
    if (window.currentMode === 1) {
        repetitionIdx++;
        if (repetitionIdx < REPETITIONS) {
            // 同じ文字で再度 UI 更新し、キーを再点滅させる
            prepareCharUI();
            return;
        }
        // 次の文字へ進むのでリセット
...
if(tempoSlider){
  tempoSlider.addEventListener('input', e=>{
    if(currentMode===3) applyBPM(parseInt(e.target.value));
  });
}
