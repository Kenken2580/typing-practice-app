...
function handleKeyPress(e){
  // ゲームが開始されていないか、IME変換中は処理しない
  if (start.disabled === false || e.isComposing) return;
  
  // 特殊キー（Shift, Ctrlなど）は処理せず無視する
  // Spaceキーは文字として扱うため除外
  const specialKeys = ['Shift', 'Control', 'Alt', 'CapsLock', 'Enter', 'Backspace', 'Tab', 'Meta', 'ContextMenu', 'NonConvert', 'Convert', 'KanaMode', 'ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End', 'PageUp', 'PageDown', 'Delete', 'Insert'];
